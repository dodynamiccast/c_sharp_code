...
namespace VodUploadTool
{
 ...
 //       List<PartUploadThread> m_arrThreadList;
        ...
        public int Init(String secId, String secKey)
        {
            m_strSecId = secId;
            m_strSecKey = secKey;
        ...
        private string GetReqSign(Dictionary<String, Object> dicVals)
        {
            if (m_iIsExReq == 1)
            {
                return m_strSigEx;
        ...
        {
            String reqStr = "";
            foreach (String key in mapVals.Keys)
            {
                if (reqStr == "")
                {
                    reqStr += '?';
...
if (ret < 0)
{
Console.WriteLine("init upload failed ret %d", ret);
Report(ret);
return ret;
}
m_arrThread = new List<UploadThread>();
for (int i = 0; i < m_iThreadNum; i++)
{
m_arrThread.Add(new UploadThread(this));
}
for (int i = 0; i < m_arrPartInfo.Count; i++)
{
m_arrThread[i % m_iThreadNum].AddIndex(i);
}
for (int i = 0; i < m_iThreadNum; i++)
{
m_arrThread[i].Strat() ;
...
for (int i = 0; i < m_iThreadNum; i++)
{
if (m_arrThread[i].m_iRet != 0)
{
  Report(m_arrThread[i].m_iRet);
  return m_arrThread[i].m_iRet;
}
}
ret = FinishUpload();
if (ret != 0)
...
m_qwEndTime = GetIntTimeStamp();
Report(0);
Console.WriteLine(m_strFileId + m_iThreadNum + " errcode " + m_iThreadUploadError);
return 0;
}
}
class UploadThread
...
List<int> m_arrIndex;
Thread m_threadRun;
VodUpload m_vodUpload;
public int m_iRet = 0;
int m_errIndex = 0;
public UploadThread(VodUpload upload)
{
m_vodUpload = upload;
m_arrIndex = new List<int>();
m_threadRun = new Thread(new ThreadStart(this.Run));
}
...
public void Run()
{
for (int i = 0; i < m_arrIndex.Count; i++)
{
m_iRet = m_vodUpload.PartUpload(m_arrIndex[i]);
if (m_iRet < 0)
{
  m_errIndex = i;
  break;
